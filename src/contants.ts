export const ROUND_TIMER = 90;

export const STARTING_PHRASE = [
  {
    text: "Wah lao eh, today weather hot until can fry egg on pavement sia.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/0482a660-4680-4995-80e6-b0f28d377918.mp3`,
  },
  {
    text: "You faster than CNA update — 1 minute the news already spread liao.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/777bc533-6b4b-4120-a1dc-f6cec0e173eb.mp3`,
  },
  {
    text: "You not kaypoh la, just very community-focused hor!",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/2a30f281-7200-4cd5-babf-08e1b7977377.mp3`,
  },
  {
    text: "Walao, this kopi peng got more ice than Marina Bay.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/ecf109d0-4479-4c51-9d46-f6fb624a5a99.mp3`,
  },
  {
    text: "Wah lao eh, MRT again spoil ah? I late again confirm kena lecture one.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/mrt.mp3`,
  },
  {
    text: "You blur like sotong sia, even Google also cannot help you already.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/google.mp3`,
  },
  {
    text: "Aiyo, my exam just now really jialat sia, like write until hallucinate already.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/exam.mp3`,
  },
  {
    text: "He walk so fast, I think he training for MRT Olympics.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/olympics.mp3`,
  },
  {
    text: "Last time you think I blur blur, now I pong three times straight. Don’t play play ah!",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/playplay.mp3`,
  },
  {
    text: "He spill kopi on boss’ laptop, then next week become team lead. Liddat also can ah?",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/liddat.mp3`,
  },
  {
    text: "Yesterday she say she single, today she post 'date night' on Insta. Wah, her pattern more than badminton sia.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/badminton.mp3`,
  },
  {
    text: "I see your Insta got A&W, massage, bak kut teh. Eh hello, bo jio!",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/bojio.mp3`,
  },
  {
    text: "Wah, you tell me you good at DIY. I thought you more zai than that! Now the shelf fall down liao.",
    audio: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/syai-mermurs/zai.mp3`,
  },
];
